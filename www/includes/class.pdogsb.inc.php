...
class PdoGsb {
     ...
     * @return null
     */
    public function majFraisHorsForfait($idVisiteur, $mois, $idFrais, $libelle, $date, $montant) {
    ...
    /**
     * Retourne le total des frais forfait pour un visiteur pour un mois
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     *
     * @return les frais forfait pour un visiteur pour un mois dans une variable
     */
    ...
    /**
     * Retourne le total des frais hors forfait pour un visiteur pour un mois
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     *
     * @return les frais hors forfait pour un visiteur pour un mois dans une variable
     */
    ...
    /**
     * Modifie le montant valider pour un visiteur pour un mois.
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     *
     * @return null
     */
    public function majMontantValide($idVisiteur, $mois, $totalFrais) {
        $requetePrepare = PdoGSB::$monPdo->prepare(
                'UPDATE fichefrais '
                . 'SET fichefrais.montantvalide = :unMontantValide '
                . 'WHERE fichefrais.idvisiteur = :unIdVisiteur '
                . 'AND fichefrais.mois = :unMois '
        );
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMois', $mois, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMontantValide', $totalFrais, PDO::PARAM_STR);
        $requetePrepare->execute();
    }

    /**
     * Retourne les informations des visiteurs dont l'etat est à VA
     *
     * @return l'id, le nom et le prénom sous la forme d'un tableau associatif
     */
    ...
    /**
     * Retourne les mois pour lesquels il y a une fiche frais dont l'etat est à VA
     *
     * @return le mois sous la forme d'un tableau associatif
     */
    public function getMoisVA() {
        $requetePrepare = PdoGsb::$monPdo->prepare(
            'SELECT fichefrais.mois AS mois FROM fichefrais '
            . 'WHERE idetat = "VA"'
            . 'ORDER BY fichefrais.mois desc'
